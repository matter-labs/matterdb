...
fn rollbacks_for_indexes_in_same_family() {
    ...
    fn indexes(fork: &Fork) -> (ListIndex<&Fork, i64>, ListIndex<&Fork, i64>) {
        let list1 = fork.get_list(("foo", &1));
        let list2 = fork.get_list(("foo", &2));
        (list1, list2)
    }
...
fn test_metadata(addr: impl Into<IndexAddress>) {
    ...
    let fork = db.fork();
    ViewWithMetadata::get_or_create(&fork, &addr, IndexType::Map)
        .map_err(drop)
        .unwrap();
    assert!(
